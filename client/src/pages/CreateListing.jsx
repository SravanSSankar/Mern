          ...
          <div className='flex gap-6 flex-wrap'>
            {['sale', 'rent', 'parking', 'furnished', 'offer'].map((id) => (
              <div key={id} className='flex gap-2'>
                <input type='checkbox' id={id} className='w-5' onChange={handleChange} checked={formData[id] === true || formData.type === id} />
          ...
          <div className='flex flex-wrap gap-6'>
            {['bedrooms', 'bathrooms', 'regularPrice'].map((id) => (
              <div key={id} className='flex items-center gap-2'>
                <input type='number' id={id} min='1' max='10000000' required className='p-3 border border-gray-300 rounded-lg' onChange={handleChange} value={formData[id]} />
                <p>{id === 'regularPrice' ? 'Regular price ($ / month)' : id.charAt(0).toUpperCase() + id.slice(1)}</p>
              </div>
            ))}
            {formData.offer && (
